...
def pe92_load_data(label_opt='cho', dont_use_cache=False, n_fold=1):
    ...
    return core_load_data(name='pe92', label_opt=label_opt, dont_use_cache=dont_use_cache, n_fold=n_fold)


def seri_load_data(label_opt='cho', dont_use_cache=False, n_fold=1):
    print("seri_load_data")
    
    return core_load_data(name='seri', label_opt=label_opt, dont_use_cache=dont_use_cache, n_fold=n_fold)


def handb_load_data(label_opt='cho', dont_use_cache=False, n_fold=1):
    print("handb_load_data")
    
    return core_load_data(name='handb', label_opt=label_opt, dont_use_cache=dont_use_cache, n_fold=n_fold)
...
def read_hgu1_folder(foldername):
    ...
    assert os.path.exists(foldername), "`%s` folder doesn't exist. You must first extract the provided zip file."%(foldername,)
    xs = []
    ys = []
